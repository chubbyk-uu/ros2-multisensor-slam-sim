...
- 周期性排除近期关键帧后按空间距离筛选历史候选，再用候选附近的多帧
  子地图执行独立的高门限相关匹配。
- 回环验证通过后添加带 Huber 鲁棒核的回环边，运行 Ceres 全图优化，
  并立即更新紫色关键帧路径和当前 `map -> odom` 校正。
...
自动回环检测和位姿图优化已经接入，但优化后尚未用全部关键帧重建
`/custom_slam/map`；因此回环后紫色位姿图路径和当前定位会先被校正，
黑白占据栅格仍保留优化前的历史观测。下一步是按优化后位姿重投影
关键帧并重建地图。
