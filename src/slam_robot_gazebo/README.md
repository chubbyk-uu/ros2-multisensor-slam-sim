...
沿视线方向、`5–15 m` 处的结构：东侧两道净空 `2.08 m` 的过顶桁架、北侧
`0.6 / 1.4 / 2.3 m` 三级阶梯货架和一块雨棚、西侧一组高度各异的柱体。
南侧 24 m 刻意只在两端各放一个矮标志，中段 21 m 没有纵向几何。

世界还包含两个高度感知导航靶标。东侧第一道门洞下沿为 `0.55 m`，高于
含 3D 雷达的 `0.35 m` 机器人和 `0.45 m` 安全扫掠上限，必须允许通过；
北侧柱体的底座半径仅 `0.07 m`，但 `z=0.18–0.44 m` 的柱帽半径为
`0.40 m`，导航必须按柱帽最大外轮廓绕行，而不能贴着细底座通过。
