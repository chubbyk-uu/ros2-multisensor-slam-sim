...
`0.0052 m / 0.502°`、`0.0177 m / 0.025°` 和
`0.0067 m / 0.134°`，地图面积均未增长。
...
前端消息间隔、拒配、错误回环、关键帧数量、地图覆盖范围和关键日志。
完全平行墙在行进方向上没有几何约束，因此纵向误差使用 `1.0 m` 的
“不得失控”上限，不把它表述为激光可观测精度；横向和航向分别使用
`0.10 m / 2.0°` 的全程上限。

2026-08-03 默认参数回归通过：真值行驶 `25.104 m`，建立 315 个关键帧，
横向峰值 `0.030 m`、航向峰值约 `0.001°`，前端最大间隔 `0.100 s`，
无拒配、无错误回环；地图范围为 `32.40 × 2.95 m`。纵向峰值
`0.855 m`，作为当前相关匹配在不可观测方向上的退化基线保留。
