...
固定输入的数据契约首先由一圈 `185.288 s / 71.6 m` 多传感器包验证：RGB、Depth、
两路 CameraInfo 与 3D LiDAR 均为 `1853` 帧，尺寸、编码、内参、frame 与精确公共
时间戳均通过检查。单圈路线只回到起点而没有以相同朝向再次走过同一路段，因此它是
传输契约样本，不是视觉回环正样本。

正式固定输入验收改用带方向性墙面纹理的两圈包：`360.608 s / 143.136 m`，RGB、
Depth 与 LiDAR 均为 `3607` 帧。RTAB-Map 的 `721` 帧诊断中，`626` 帧观测到视觉词，
接受 `178` 次视觉回环和 `280` 次空间邻近约束；位置 RMSE/峰值/末端误差为
`0.271/0.699/0.062 m`。同一包内输入里程计的 RMSE/末端误差为 `0.642/1.263 m`，
因此该次结果证明视觉图优化确实修正了自身输入漂移。处理 P95 为 `82.773 ms`，
平均 CPU `30.9%`，峰值 RSS `1272.8 MiB`；七项硬判据全部通过。纹理只改变视觉
材质，没有改变碰撞和 LiDAR 几何。结果见
...
完整在线验收在 Gazebo 中现场生成默认 `640 × 480 @ 30 Hz` RGB-D，并让机器人主动
行驶两圈。轻量 CameraInfo 计数为 `11079`，按消息时间戳实测 `30.304 Hz`；驾驶器
8/8 航点完成，17 项路线、闭环、地图、TF 和运行时检查全部通过。RTAB-Map 接受
`134` 次视觉回环与 `205` 次空间邻近约束，位置 RMSE/峰值/末端误差为
`0.093/0.218/0.031 m`，航向 RMSE `0.441°`。同次输入里程计为 `0.237/0.416 m`
（RMSE/末端），满足漂移改善判据。

在线处理均值/P95 为 `27.866/64.734 ms`，`rgbd_sync + rtabmap` 平均 CPU
`31.2%`、峰值 RSS `1085.0 MiB`，数据库 `190.2 MB`；地图含 `73027` 个自由格和
`3516` 个占据格。八项在线汇总判据全部通过。判定完成后的 Gazebo `exit -11` 属于
...
最终组合入口还完成了动态障碍端到端验收。前向 RGB-D 先以三个已知自由区目标渐进
建图并返回原点，随后在不改变公共动态障碍几何和判据的前提下导航到 `(-4,-1)`。
三个预建图目标全部成功；正式导航 `15.1 s` 到达，3D LiDAR 在局部/全局代价地图
中的标记时延为 `0.160/0.320 s`，车体净空 `0.239 m`、障碍局部绕行偏移
`1.133 m`、终点误差 `0.137 m`，零恢复、零碰撞监视器介入。RTAB-Map 的
`map -> odom` 容差按 `2 Hz` 图更新周期推导为 `0.60 s`，避免 Nav2 在两次视觉
更新之间请求未来 TF。结果见
[`2026-09-04-rtabmap-rgbd-lidar-navigation.json`](results/2026-09-04-rtabmap-rgbd-lidar-navigation.json)。

这项组合验收不替代上面的两圈视觉精度与资源验收；它补齐的是同一进程组合中
“RGB-D 在线建图/定位 + 3D LiDAR 动态避障”的安全行为证据。

## 启动与故障注入验收

十个复合 3D launch 入口均有无界面冒烟检查，验证作用域、关键节点持续在线、
