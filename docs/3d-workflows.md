...
承担不同职责，是项目最终采用的松耦合架构，不做紧耦合融合。地图形成后使用 RViz
顶部 `Nav2 Goal`
...
证明判据确实区分“路线上出现障碍”和“门洞真正封死”。动态绕行只统计障碍纵向站位
附近的轨迹，默认门限是机器人与障碍外接圆之和 `0.760 m`；横移负向对照必须让
`detour_observed` 本身失败，不能只靠代价地图未观察到障碍而失败。封路沿用专用单门洞世界，不在
...
最终 RGB-D 导航入口采用松耦合职责分工：RTAB-Map 使用 RGB-D 与轮速 + IMU
`/odom` 完成建图和定位，其深度二维地图供 Nav2 全局规划；3D LiDAR 不进入
RTAB-Map，只供 Nav2 障碍层和碰撞监视器使用。现有
`rtabmap_rgbd_navigation_simulation.launch.py` 即为最终组合入口，而非等待融合的
临时对照。

自研 3D LiDAR SLAM 和 RTAB-Map 纯 LiDAR 入口继续独立保留。当前不再接入视觉
回环候选、视觉几何约束、RGB-D 补充障碍或双目，也不计划融合 2D 与 3D LiDAR。
完整 LIO 仍需要当前 Gazebo 点云不具备的逐点时间数据，因此同样不在当前范围。
