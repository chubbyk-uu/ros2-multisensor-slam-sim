...
当前格式为版本 3，每个关键帧分别保存 `0.10 m` 注册点云和 `0.05 m` 占据点云。
版本 1/2 加载时明确报错并要求重新建图。降级读取是做得到的——旧文件里那份
`0.10 m` 点云可以同时充当两份，恢复出的占据栅格与本次改动之前逐格相同——
但项目当前没有外部使用者、存量快照只有个位数，为此长期维护一条兼容分支不
划算。这是一个明确的取舍，不是"数据不足以恢复"：代价是升级即作废全部已有
快照，节点会 FATAL 退出而非降级。恢复时全部历史关键帧的前端位姿会被刚体变换到地图系，否则跨越
接缝的第一条位姿图边会把整个累积修正当成一次测量。

占据点云在平面模式下只保存最大投影距离内的地面自由射线和高度带内障碍点；
高于障碍带的回波在二维投影中既不清自由空间也不标障碍，因此不会进入关键帧和
快照。距离和高度分类使用同一个投影函数，不再由节点和栅格各自实现。v3 文件还
...
日常入口的 `exploration_seed:=0` 会为每次运行生成新种子，只在完成 Nav2 路径
验证且位于本轮评分顶部区间的候选中随机选择，不会绕过可达性或安全代价。需要
复现路线时传入非零种子，例如 `exploration_seed:=42`；自动回归使用固定种子。
